...
case $DISTRO in
    "fedora"|"rhel"|"centos"|"rocky"|"almalinux")
        if command -v dnf >/dev/null 2>&1; then
            dnf install gcc-toolset-13 python3.11-devel python3.11-pip gcc gcc-c++ gcc-gfortran xz cmake yum-utils openssl-devel bzip2-devel bzip2 libffi-devel \
            zlib-devel autoconf automake libtool cargo \
            pkgconf-pkg-config -y --skip-broken --nobest
            
            source /opt/rh/gcc-toolset-13/enable
        else
            sudo yum install gcc-toolset-13 python3.11-devel python3.11-pip gcc gcc-c++ gcc-gfortran xz cmake yum-utils openssl-devel bzip2-devel bzip2 libffi-devel \
            zlib-devel autoconf automake libtool cargo \
    ...
    "sles")
        sudo zypper refresh
        sudo zypper install -y gcc gcc-fortran python311 python311-pip python311-devel gcc-c++
